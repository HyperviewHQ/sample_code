...
$BaseWorkUri = [string]::Format("{0}/api/asset/alarmEvents/bulkAcknowledgementStates", $HostName);
...
# Read CSV File
$CsvData = Import-Csv -Path ./data/alarm_ids_to_acknowledge.csv
...
foreach($WorkBatch in $WorkQueue)
{
    ...
    {
        $BulkRequestPayload.alarmEventIds += $Id;
    }
