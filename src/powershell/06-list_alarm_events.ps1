<#
  ...
  .PARAMETER AllEvents
    Optional parameter to specify where to list all alarm events (Unacknowledged and Acknowledged).
...
param (
    ...
    [Parameter(HelpMessageBaseName="Specify if only unacknowledged or all events should be fetched")]
    [switch] $AllEvents
)
...
if (-not $AllEvents)
{
    $Parameters['filter'] = '["acknowledgementState","=","unacknowledged"]';
}
